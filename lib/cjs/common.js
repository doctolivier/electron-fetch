...
function convert(str = "", to, from) {
  ...
  if (from === to) {
    ...
    try {
      if (to === "UTF-8") {
        result = import_iconv_lite.default.decode(str, from);
      } else if (from === "UTF-8") {
        result = import_iconv_lite.default.encode(str, to);
      } else {
        result = import_iconv_lite.default.encode(import_iconv_lite.default.decode(str, from), to);
      }
    } catch (err) {
      console.error(err);
      result = str;
...
function writeToStream(dest, instance) {
  ...
  if (body === null) {
    ...
    body.pipe(new import_node_stream.PassThrough()).pipe(dest);
  }
}
